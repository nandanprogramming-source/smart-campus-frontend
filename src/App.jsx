...
/* ------------------------------------------------------------------ */
/* Campus Navigation — the flagship module                             */
/* ------------------------------------------------------------------ */
function BuildingMarker({ b, selected, isDest, dark, onSelect }) {
  const catStyle = CATEGORY_STYLE[b.category];
  const Icon = catStyle.icon;
  return (
    <g transform={`translate(${b.x},${b.y})`} className="cursor-pointer" onClick={() => onSelect(b)}>
      <rect
        x="-52" y="-26" width="104" height="52" rx="16"
        fill={dark ? 'rgba(28,28,30,0.9)' : 'rgba(255,255,255,0.92)'}
        stroke={selected || isDest ? PALETTE.appleBlue : 'transparent'}
        strokeWidth="2"
      ...
      <foreignObject x="-52" y="-26" width="104" height="52">
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, height: '100%', padding: '0 8px' }}>
          <div style={{ width: 24, height: 24, borderRadius: 8, flexShrink: 0, background: `linear-gradient(135deg, ${catStyle.grad[0]}, ${catStyle.grad[1]})`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Icon size={12} color="#fff" />
          </div>
          <span style={{ fontSize: 9.5, fontWeight: 600, color: dark ? '#F5F5F7' : '#1C1C1E', lineHeight: 1.15 }}>{b.label}</span>
        </div>
      </foreignObject>
    </g>
  );
...
function CampusPage() {
  ...
  const [favorites, setFavorites] = useState(['library', 'cafeteria']);
  const [liveRoute, setLiveRoute] = useState(null);
  const [routeLoading, setRouteLoading] = useState(false);
  const [routeError, setRouteError] = useState('');

  const filtered = query ? NAV_BUILDINGS.filter((b) => b.label.toLowerCase().includes(query.toLowerCase())) : [];

  // Demo mode: compute the shortest path locally against the frontend's own graph.
  const demoRoute = useMemo(() => {
    if (!destination || session) return null;
    const r = shortestPath('gate', destination, accessible);
    const path = r.path.map((id) => NAV_POINT_MAP[id]);
    return { path, distance: r.distance, estimatedMinutes: Math.max(1, Math.round(r.distance / 140)) };
  }, [destination, accessible, session]);

  // Live mode: ask the real backend, which runs the same Dijkstra algorithm
  // against the database (the node ids match 1:1 with the seeded graph).
  useEffect(() => {
    if (!destination || !session) { setLiveRoute(null); return; }
    let cancelled = false;
    setRouteLoading(true);
    setRouteError('');
    api.campusRoute(session.accessToken, 'gate', destination, accessible)
      .then((res) => { if (!cancelled) setLiveRoute(res); })
      .catch((err) => { if (!cancelled) { setLiveRoute(null); setRouteError(err.message || 'Could not compute a route'); } })
      .finally(() => { if (!cancelled) setRouteLoading(false); });
    return () => { cancelled = true; };
  }, [destination, accessible, session]);

  const route = session ? liveRoute : demoRoute;
  const routePoints = route ? route.path : null;
  const steps = route ? buildStepsFromPoints(route.path) : [];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl sm:text-2xl font-bold" style={{ color: c.text }}>Campus Navigation</h1>
        <p className="text-sm mt-1" style={{ color: c.subtext }}>Find your way around UVCE</p>
      </div>

      <div className="grid lg:grid-cols-4 gap-6">
        <div className="space-y-4 order-2 lg:order-1 lg:col-span-1">
          <Glass className="p-4">
            <div className="flex items-center gap-2 px-3 py-2 rounded-full mb-3" style={{ background: c.panelSoft }}>
              <Search size={14} style={{ color: c.faint }} />
              <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search a building…" className="bg-transparent outline-none text-sm flex-1" style={{ color: c.text }} />
            </div>
            {query && (
              <div className="space-y-1 mb-2">
                {filtered.map((b) => (
                  <button key={b.id} onClick={() => { setSelected(b); setQuery(''); }} className="w-full text-left px-3 py-2 rounded-xl text-sm hover:bg-black/5" style={{ color: c.text }}>
                    {b.label}
                  </button>
                ))}
                {filtered.length === 0 && <p className="text-xs px-3" style={{ color: c.faint }}>No matches</p>}
              </div>
            )}
            <label className="flex items-center gap-2 px-1 pt-2 text-xs" style={{ color: c.subtext, borderTop: `1px solid ${c.divider}` }}>
          ...
          <Glass className="p-4">
            <p className="text-xs font-semibold mb-3" style={{ color: c.text }}>Frequently visited</p>
            <div className="space-y-1">
              {favorites.map((id) => {
                const b = NAV_BUILDINGS.find((x) => x.id === id);
                if (!b) return null;
                const Icon = CATEGORY_STYLE[b.category].icon;
                return (
                  <button key={id} onClick={() => setSelected(b)} className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-xl hover:bg-black/5">
                    <Icon size={14} style={{ color: PALETTE.appleBlue }} />
                    <span className="text-sm flex-1 text-left" style={{ color: c.text }}>{b.label}</span>
                    <Star size={13} fill={PALETTE.amber} stroke={PALETTE.amber} />
                  </button>
                );
              })}
            </div>
          ...
          {!selected && !route && (
            <Glass className="p-6 text-center">
              <MapIcon size={28} className="mx-auto mb-2" style={{ color: c.faint }} />
              <p className="text-sm font-medium" style={{ color: c.text }}>Tap a building to begin</p>
              <p className="text-xs mt-1" style={{ color: c.faint }}>Or search above to find a location</p>
            </Glass>
          )}
          ...
          {selected && (
            <Glass className="p-4">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-sm font-semibold" style={{ color: c.text }}>{selected.label}</p>
                  <p className="text-xs mt-1 capitalize" style={{ color: c.faint }}>{selected.category}</p>
                </div>
                <button onClick={() => toggleFavorite(selected.id)}>
                  <Star size={16} fill={favorites.includes(selected.id) ? PALETTE.amber : 'none'} stroke={PALETTE.amber} />
                </button>
              </div>
              <p className="text-xs mt-2.5" style={{ color: c.subtext }}>{selected.blurb}</p>
              <p className="text-xs mt-2 flex items-center gap-1.5" style={{ color: c.faint }}><Clock size={12} /> {selected.hours}</p>
              <button onClick={() => setDestination(selected.id)} className="w-full mt-3.5 py-2 rounded-full text-xs font-semibold text-white flex items-center justify-center gap-1.5" style={{ background: `linear-gradient(135deg, ${PALETTE.appleBlue}, #38BDF8)` }}>
          ...
          {routeLoading && (
            <Glass className="p-4 text-center">
              <p className="text-xs" style={{ color: c.faint }}>Computing route…</p>
            </Glass>
          )}
          {routeError && (
            <Glass className="p-4 text-center">
              <p className="text-xs" style={{ color: PALETTE.softRed }}>{routeError}</p>
            </Glass>
          )}
          {route && (
            <Glass className="p-4">
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-semibold" style={{ color: c.text }}>Directions</p>
                <span className="text-xs font-semibold" style={{ color: PALETTE.appleBlue }}>{route.estimatedMinutes} min</span>
              </div>
              <ol className="space-y-2">
                {steps.map((s, i) => (
                  <li key={i} className="text-xs flex gap-2" style={{ color: c.subtext }}>
                    <span className="w-4 h-4 rounded-full flex items-center justify-center text-xs font-bold shrink-0 mt-0.5" style={{ background: c.panelSoft, color: c.text }}>{i + 1}</span>
                    {s}
                  </li>
                ))}
              </ol>
          ...
          <svg viewBox="0 0 920 620" className="w-full h-auto min-w-[500px] sm:min-w-0" style={{ maxHeight: 560 }}>
            <defs>
              <style>{`
                ...
                .nav-path-route { stroke: ${PALETTE.appleBlue}; stroke-width: 5; fill: none; stroke-linecap: round; stroke-linejoin: round; }
                @keyframes navDraw { to { stroke-dashoffset: 0; } }
                @keyframes navPulse { 0%, 100% { r: 8; opacity: 0.35; } 50% { r: 14; opacity: 0; } }
              `}</style>
            </defs>

            {NAV_EDGES.map((e, i) => {
              const a = NAV_POINT_MAP[e.from]; const b = NAV_POINT_MAP[e.to];
              return <line key={i} x1={a.x} y1={a.y} x2={b.x} y2={b.y} className="nav-path-bg" strokeDasharray={e.stairs ? '2 8' : undefined} />;
            })}

            {routePoints && (
              <polyline
                key={`${destination}-${accessible}`}
                points={routePoints.map((p) => `${p.x},${p.y}`).join(' ')}
                className="nav-path-route"
                style={{ strokeDasharray: route.distance + 20, strokeDashoffset: route.distance + 20, animation: 'navDraw 1.1s ease forwards' }}
              />
            )}

            {NAV_AMENITIES.map((a) => (
              <g key={a.id} transform={`translate(${a.x},${a.y})`}>
                <circle r="9" fill={dark ? '#1C1C1E' : '#fff'} stroke={CATEGORY_STYLE.amenity.grad[0]} strokeWidth="2" />
                <circle r="3.5" fill={CATEGORY_STYLE.amenity.grad[0]} />
              </g>
            ))}

            {NAV_BUILDINGS.map((b) => (
              <BuildingMarker key={b.id} b={b} dark={dark} selected={selected && selected.id === b.id} isDest={destination === b.id} onSelect={setSelected} />
            ))}

            <g transform={`translate(${NAV_POINT_MAP.gate.x},${NAV_POINT_MAP.gate.y})`}>
              <circle r="14" fill="none" stroke={PALETTE.appleBlue} strokeWidth="2" style={{ animation: 'navPulse 1.8s ease-out infinite' }} />
              <circle r="7" fill={PALETTE.appleBlue} stroke="#fff" strokeWidth="2" />
            </g>
          </svg>
        </Glass>
      </div>
